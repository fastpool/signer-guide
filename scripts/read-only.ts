 ...
 *
 * `scripts/locked.ts` keeps its own copy of this against `src/lib/clarity.ts`,
 * because it is paired with a hand-rolled encoder it must not outgrow. These
 * two are free to use the real library, so they share this instead.
 */
 ...
 *
 * Long enough to outlast a per-minute window, because that is the limit these
 * scripts actually hit. A walk of ten thousand pages that gives up two thirds
 * of the way through has not saved anybody time — it has produced a number
