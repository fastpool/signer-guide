    ...
    <main className='mx-auto max-w-3xl px-5 py-12 md:py-20'>
      <header className='flex flex-col gap-4'>
        ...
        <h1 className='text-4xl font-extrabold md:text-5xl'>
          {ko
            ? '내 STX를 어디에 스테이킹할 수 있을까?'
            : 'Where can you stake your STX?'}
        </h1>
        <p className='text-lg text-muted'>
          {ko ? (
            <>
              스테이킹할 때는 내 자산을 맡길 풀을 고르게 됩니다. 지금 선택
              가능한 풀은 <strong className='text-ink'>{total}개</strong>지만,
              실제로는{' '}
            ...
            <>
              When you stake, you pick a pool to look after it for you. There
              are <strong className='text-ink'>{total} pools</strong> to choose
              from today, but between them they run only{' '}
              <strong className='text-ink'>
      ...
      <footer className='mt-12 space-y-3 border-t border-black/5 pt-6 text-sm text-muted'>
        <p>
          {ko ? (
            <>
              <strong className='text-ink'>수수료 안내.</strong> 표시된 수수료는
              각 풀의 컨트랙트에서 읽은 현재 적용 값입니다. 대부분의 풀은
              수수료를 고정하지 않기 때문에 나중에 바꿀 수 있습니다. 일부
              컨트랙트는 코드로 상한을 두며, 이런 경우 <em>수수료 상한</em>{' '}
              배지가 붙습니다. 또 더 적은 일부는 수수료 변경 전에 대기 기간을
              두어 이동할 시간을 주며, 이런 경우 <em>수수료 변경 사전 공지</em>{' '}
              배지가 붙습니다. 어떤 풀은 이 컨트랙트에 수수료 정의가 전혀 없기도
              하며, 그 경우 무료라는 뜻은 아닙니다.
            </>
          ) : (
            <>
              <strong className='text-ink'>About the fees.</strong> The fee
              shown is the one in force right now, read from the pool&rsquo;s
              own contract. Most pools do not lock their fee in, so they can
              change it later. A few contracts do set a ceiling in code — those
              carry a <em>fee capped</em> badge, and that limit holds whatever
              the pool decides. Fewer still make a fee change wait before it
              applies, which gives you time to move — those carry a{' '}
              <em>fee changes announced</em> badge. Some pools have no fee in
              this contract at all, which does not always mean free, because the
              fee may be taken elsewhere.
            </>
          )}
        </p>
        <p>
