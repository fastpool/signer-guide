...
describe('nodesBySignerKey', () => {
  it('makes one signer of the contracts sharing a key', () => {
    const nodes = nodesBySignerKey([
      signer('SP1.one', KEY_A),
      signer('SP1.two', KEY_B),
      signer('SP1.three', KEY_A),
    ]);
    expect(nodes).toHaveLength(2);
    expect(nodes[0].contracts.map((c) => c.contractId)).toEqual([
      'SP1.one',
      'SP1.three',
    ]);
    expect(nodes[1].contracts.map((c) => c.contractId)).toEqual(['SP1.two']);
  });
  ...
  it('never merges two contracts on a key neither of them has', () => {
    ...
    // another's.
    const nodes = nodesBySignerKey([signer('SP1.one'), signer('SP1.two')]);
    expect(nodes).toHaveLength(2);
    expect(nodes.every((g) => g.signerKey === null)).toBe(true);
  });

  it('finds the signer a contract belongs to, siblings attached', () => {
    const signers = [signer('SP1.one', KEY_A), signer('SP1.two', KEY_A)];
    const node = nodeForContract(signers, 'SP1.two');
    expect(node?.contracts).toHaveLength(2);
    expect(nodeForContract(signers, 'SP1.nope')).toBeNull();
  });
});
...
describe('signerSlug', () => {
  ...
  it('gives every signer in the real data its own slug', () => {
    const nodes = nodesBySignerKey((realData as SignerData).signers);
    const slugs = nodes.map(signerSlug);
  ...
  it('gives every slug a name a path can hold', () => {
    ...
    for (const slug of nodesBySignerKey((realData as SignerData).signers).map(
      signerSlug,
    )) {
...
describe('votingPowerBips', () => {
  ...
  it('weighs the key against the whole cycle, not against itself', () => {
    ...
    expect(votingPowerBips(node('SP1.one', 'SP1.two'), ustx)).toBe(5_000);
    expect(votingPowerBips(node('SP1.one'), ustx)).toBe(2_500);
  });

  it('adds up every contract on the key', () => {
    const ustx = { 'SP1.one': '250', 'SP1.two': '250', 'SP2.other': '500' };
    expect(nodeUstx(node('SP1.one', 'SP1.two'), ustx)).toBe(500n);
  });
  ...
  it('is unknown, not zero, when an amount could not be read', () => {
    ...
    expect(votingPowerBips(node('SP1.one'), ustx)).toBeNull();
    expect(nodeUstx(node('SP1.one'), ustx)).toBeNull();
  });
  ...
  it('counts what it could read when only a sibling is missing', () => {
    ...
    expect(votingPowerBips(node('SP1.one', 'SP1.two'), ustx)).toBe(2_500);
  });

  it('says nothing rather than dividing by an empty cycle', () => {
    expect(votingPowerBips(node('SP1.one'), { 'SP1.one': '0' })).toBeNull();
    expect(votingPowerBips(node('SP1.one'), {})).toBeNull();
  });
  ...
  it('keeps its precision on the amounts pox-5 actually holds', () => {
    ...
    expect(votingPowerBips(node('SP1.one'), ustx)).toBe(1_961);
  });
});
...
describe('the real signers', () => {
  it('never add up to more than the whole cycle between them', () => {
    ...
    const total = nodesBySignerKey(signers).reduce(
      (sum, node) => sum + (votingPowerBips(node, ustx) ?? 0),
      0,
    );
