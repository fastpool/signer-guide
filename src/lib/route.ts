 ...
 *   #/bonds                  the bitcoin side: what the current and next bond
 *                            period hold, and who was invited to them
 *
...
export function parseHash(hash: string): Route {
  ...
  if (hash === '#/bonds' || hash === '#/bonds/') return { name: 'bonds' };
  ...
  // Before the single-group route, which it is not a prefix of but reads like
  // one. The plural is the index; the singular takes an id.
  if (hash === '#/groups' || hash === '#/groups/') return { name: 'groups' };
...
/** A link to the index of every group. */
export function groupsHref(): string {
  return '#/groups';
}
...
/** A link to the bitcoin side: what the current and next bond hold. */
export function bondsHref(): string {
  return '#/bonds';
}
