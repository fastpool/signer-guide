 ...
 * to reach for, but what comes back is used to look up somebody's stake, so
 * resolving to a stale owner would report one person's position under another
 * person's name — silently, and with nothing on screen to suggest it. The
 * registry is the thing that decides who owns a name, so the registry is what
 * is asked.
 *
 ...
 *
 * The node URL is a parameter rather than a module constant, which is what
 * lets the phone app use this: `bns.ts` supplies the browser's build-time
 * default, and the app passes its own.
 */
...
/** The registry. Mainnet only, like the rest of the guide. */
...
export async function resolveBnsName(
  name: string,
  opts: { apiUrl: string; signal?: AbortSignal },
): Promise<BnsResolution> {
  const { apiUrl, signal } = opts;
  ...
  try {
    const res = await fetch(
      `${apiUrl}/v2/contracts/call-read/${address}/${contract}/can-resolve-name`,
      {
        method: 'POST',
