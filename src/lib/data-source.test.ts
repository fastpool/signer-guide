...
describe('what a saved copy has to look like to be believed', () => {
  const write = (value: unknown) =>
    localStorage.setItem('signer-guide:snapshot:v2', JSON.stringify(value));
  ...
  it('refuses one that is not JSON at all', () => {
    localStorage.setItem('signer-guide:snapshot:v2', 'half a wr');
    expect(readCachedSnapshot()).toBeNull();
  });
});
...
describe('reading the branch', () => {
      ...
      vi.fn(async (url: string) => ({
        ...
        json: async () =>
          url.includes('signers')
            ? signers
            : url.includes('stx-only-calculations')
              ? stxOnlyCalculations
              : totals,
      })),
    );
  ...
  it('takes both files as one snapshot', async () => {
    respond(
      { ...BUNDLED.signers, generatedAt: LATER },
      BUNDLED.totals,
      BUNDLED.stxOnlyCalculations,
    );
  ...
  it('refuses a published file this build cannot read', async () => {
    ...
    respond({ generatedAt: LATER }, BUNDLED.totals, BUNDLED.stxOnlyCalculations);
    await expect(fetchSnapshot()).rejects.toThrow(/shape/);
  });

  it('fails rather than treating an error page as data', async () => {
    respond(BUNDLED.signers, BUNDLED.totals, BUNDLED.stxOnlyCalculations, false);
